...
> **并肩山谷 override (Path D, lean):** Brainbird delegated full custody. Do not stop for a director gate on every file. Existing `docs/WORLD.md`, `docs/CHARTER.md`, and `godot/docs/ART.md` are authority. Do not run `/brainstorm`. See `CLAUDE.md` and `production/review-mode.txt`.
