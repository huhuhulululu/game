...
- Story: `production/epics/r1-coat-feel/story-024-sleep-sit.md` — Sleep and sit
- Files changed: story, `world.ts` reachAt spawn, `valley_logic.gd` bed here-tile, `play.gd` 歇一夜 slip, `play_rest.tscn`, `headless_play_rest.gd`, ART, control-manifest, `godot/project.test.ts`
- Test written: `godot/project.test.ts` (sleep-sit lock) + `play_rest.tscn`
- Tokens: `PLAY_SLEEP` `PLAY_SIT` `PLAY_REST_OK`
- Next: `/story-done` after tokens hold. Existing sleep rules. Existing sit sheet. No bed or fire sticker. 023 stays Complete. 019 stays parked. Stop new WORLD systems. Two-phone stays Blocked (human).
...
- Criteria: 6/6. 做 on the stream / dock tile starts the existing fish pull. 做 on the out-gate enters `bed-wild`. No rod or gate sticker.
- Tests: `godot/project.test.ts` water-wild lock + `PLAY_WATER_OK`. `PLAY_ENTER_OK` still holds.
- Review: lean (QL-TEST-COVERAGE skipped; LP-CODE-REVIEW skipped)
- Next recommended: none. Stop. Do not generate a new face. Fireside 019 stays parked. Two-phone stays Blocked (human).
...
- Criteria: 6/6. 做 on the kitchen / mine tile enters. Existing beds sit. No door sticker.
- Tests: `godot/project.test.ts` enter-place lock + `PLAY_ENTER_OK`. `PLAY_HANDS_OK` still holds.
- Review: lean (QL-TEST-COVERAGE skipped; LP-CODE-REVIEW skipped)
- Next recommended: none. Stop. Do not generate a new face. Fireside 019 stays parked. Two-phone stays Blocked (human).
...
- Criteria: 6/6. Existing coats take a warm umber/gold grade. Same idle face. Bed grade stays 0.0. Not a blob.
- Tests: `godot/project.test.ts` coat-dusk lock + `PLAY_DUSK_OK`. `PLAY_IN_OK` still holds.
- Review: lean (QL-TEST-COVERAGE skipped; LP-CODE-REVIEW skipped)
- Next recommended: none. Stop. Do not generate a new face. 019 stays parked. Two-phone stays Blocked (human).
