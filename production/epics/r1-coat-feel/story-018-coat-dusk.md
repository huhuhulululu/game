...
- [ ] Existing `char-warm` / `char-pine` sheets take a warm dusk grade; same idle face; no new person
- [ ] Hard cut softens into the path; contact shadow stays; BODY stays 240
- [ ] Bed `dusk.gdshader` grade stays `0.0`; no full-screen orange grade; title couple not pasted
- [ ] If the grade reads as a blob or worse, revert and say so
- [ ] `bed-valley.png` not replaced; stickers stay off; no 魂; no look names in `src/`
- [ ] Play-path + test locks; 017 Complete; 019 parked; two-phone Blocked
