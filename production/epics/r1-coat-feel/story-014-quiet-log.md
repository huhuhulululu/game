...
- [x] Empty bottom prompt bar is hidden; no reserved empty box
- [x] A real line is one small wood slip (`tex-slip`), not a long DST log
- [x] Title stays the painted cover; hands stay small slips; BODY stays 192
- [x] HUD stays plaque + one bag slip; torn props stay off
- [x] `bed-valley.png` not replaced; no 魂; no look names in `src/`
- [x] Play-path + test locks; 013 stays Complete
