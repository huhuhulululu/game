#!/usr/bin/env python3
"""Wild camp pot. Does not touch the cover. Magenta-key prop, real a=0."""
...
def paint_camp_pot(one: object) -> Image.Image:
    W, H = 180, 200
    im = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    body = plank_box(one, (88, 78), True)
    ba = np.asarray(body, dtype=np.float32)
    ba[:, :, :3] = ba[:, :, :3] * np.array([0.38, 0.32, 0.26]) + np.array([16.0, 12.0, 8.0])
    body = Image.fromarray(np.clip(ba, 0, 255).astype(np.uint8), "RGBA")
    body.putalpha(one.ellipse_mask((88, 78), ry=0.78, blur=2).point(lambda v: min(255, int(v * 1.2))))
    im.alpha_composite(body, (46, 82))
    rim = plank_box(one, (100, 22), True)
    ra = np.asarray(rim, dtype=np.float32)
    ra[:, :, :3] = ra[:, :, :3] * np.array([0.52, 0.42, 0.30]) + np.array([20.0, 14.0, 6.0])
    rim = Image.fromarray(np.clip(ra, 0, 255).astype(np.uint8), "RGBA")
    rim.putalpha(one.ellipse_mask((100, 22), ry=0.55, blur=1.6))
    im.alpha_composite(rim, (40, 78))
    ear = plank_box(one, (18, 24), False)
    ear.putalpha(one.ellipse_mask((18, 24), ry=0.70, blur=1.2))
    im.alpha_composite(ear, (30, 100))
    im.alpha_composite(ear, (132, 100))
    stew = one.paper_sheet((64, 18), ochre=True).convert("RGBA")
    sa = np.asarray(stew, dtype=np.float32)
    sa[:, :, :3] = sa[:, :, :3] * np.array([1.08, 0.70, 0.34]) + np.array([24.0, 6.0, 0.0])
    stew = Image.fromarray(np.clip(sa, 0, 255).astype(np.uint8), "RGBA")
    stew.putalpha(one.ellipse_mask((64, 18), ry=0.62, blur=1.2))
    im.alpha_composite(stew, (58, 84))
    glow = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    d = ImageDraw.Draw(glow)
    d.ellipse((62, 72, 118, 112), fill=(252, 186, 86, 80))
    glow.putalpha(glow.getchannel("A").filter(ImageFilter.GaussianBlur(6)))
...
def main() -> None:
    ...
    one = _one()
    save_prop(paint_camp_pot(one), "prop-camp-pot.png", 0.14)
