...
func _panel() -> void:
	...
	panel.add_theme_stylebox_override("panel", Look.room_box())
	add_child(panel)
	...
	sides.size = Vector2(384, 54)
	sides.add_theme_constant_override("separation", 16)
	panel.add_child(sides)
	_warm = Look.chip_button("暖", 180)
	_pine = Look.chip_button("松", 180)
