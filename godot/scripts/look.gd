...
const FOG_MIST := Color(0.86, 0.78, 0.64)
...
const COAT_DUSK := Color(1.12, 0.76, 0.46)
const COAT_HAZE := Color(0.62, 0.40, 0.20)
...
static func fog_mat(amount := 0.48) -> ShaderMaterial:
	var m := ShaderMaterial.new()
	m.shader = load("res://shaders/fog.gdshader") as Shader
	m.set_shader_parameter("amount", amount)
	m.set_shader_parameter("mist", FOG_MIST)
	m.set_shader_parameter("shade", FOG_SHADE)
	return m
...
static func person_mat() -> ShaderMaterial:
	...
	m.set_shader_parameter("grade", 0.58)
	m.set_shader_parameter("fog", 0.07)
	m.set_shader_parameter("edge", 0.12)
	m.set_shader_parameter("feet", 0.06)
	return m
